...
private:
    ...
    [[nodiscard]] bool      scene_intersect(const Ray& ray, const Sphere*& h_sphere, Vector3& hit) const;
                  void      write_ppm(const std::string PATH, const T_COLOR& pixels) const;
                  void      write_ppm(const std::string& PATH, const T_COLOR& pixels, const int& width, const int& height) const;
...
private:
    ...
    const float _pixel_spacing = 1;                // Basically resolution of image
    const float _SSAA_factor = 25;                  // How much to upsample original image. more -> more time and better image
    ...
    const std::string _FILE_PATH = "history/image0";
